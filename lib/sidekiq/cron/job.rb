  ...
  module Cron
    class Job
      ...
      def add_jid_history(jid)
        jid_history = {
          jid: jid,
          enqueued: @last_enqueue_time.strftime(LAST_ENQUEUE_TIME_FORMAT)
        }
      ...
      def hash_to_redis hash
        hash[:last_enqueue_time] = hash[:last_enqueue_time]&.strftime(LAST_ENQUEUE_TIME_FORMAT) rescue nil
